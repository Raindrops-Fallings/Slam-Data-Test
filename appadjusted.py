...
class InstanceData(object):
    ...
    def to_features(self):
        ...
        return to_return
def get_retention_intervals(data, target_user, target_word):
    days_seen = []
    
    for instance in data:
        if instance.user == target_user and instance.token.lower() == target_word.lower():
            # Count every instance (even duplicates) by logging days
            days_seen.append(instance.days)
    
    # Sort by appearance order (already guaranteed if `data` is top-down)
    intervals = []
    for i in range(len(days_seen) - 1):
        intervals.append(days_seen[i+1] - days_seen[i])
    
    return intervals

data = load_data("EnglishDev.txt")
intervals = get_retention_intervals(data, target_user="lTIfCaa+", target_word="books")
print("Retention intervals for 'dog':", intervals)
